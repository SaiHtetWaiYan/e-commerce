        ...
        <div class="flex flex-col md:flex-row gap-6">
            ...
            <div class="hidden md:block w-[240px] flex-shrink-0">
                <div class="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-2xl shadow-sm sticky top-24 overflow-hidden">
                    ...
                    <form method="GET" action="{{ route('storefront.products.index') }}" class="p-5 space-y-6">
                        @if(request('q'))
                                ...
                                <div class="space-y-2 max-h-48 overflow-y-auto">
                                    @foreach ($categories as $cat)
                                        <label class="flex items-center gap-2 cursor-pointer group">
                                            <input type="radio" name="category" value="{{ $cat->slug }}" @checked(request('category') === $cat->slug) class="h-4 w-4 rounded border-gray-300 dark:border-gray-600 text-primary-600 focus:ring-primary-500">
                                            <span class="text-sm text-gray-700 dark:text-gray-300 group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors flex-1">{{ $cat->name }}</span>
                                            <span class="text-[10px] font-bold text-gray-400 dark:text-gray-500 bg-gray-100 dark:bg-gray-800 rounded-full px-1.5">{{ $cat->products_count }}</span>
                                        </label>
                                    @endforeach
                                </div>
                            </div>
                            ...
                            <div class="border-t border-gray-100 dark:border-gray-800 pt-5">
                                ...
                                <div class="space-y-2 max-h-48 overflow-y-auto">
                                    @foreach ($brands as $brand)
                                        <label class="flex items-center gap-2 cursor-pointer group">
                                            <input type="radio" name="brand" value="{{ $brand->slug }}" @checked(request('brand') === $brand->slug) class="h-4 w-4 rounded border-gray-300 dark:border-gray-600 text-primary-600 focus:ring-primary-500">
                        ...
                        <div class="border-t border-gray-100 dark:border-gray-800 pt-5">
                            ...
                            <div class="flex items-center gap-2">
                                <div class="relative flex-1">
                                    ...
                                    <input type="number" name="min_price" value="{{ request('min_price') }}" placeholder="Min" class="w-full border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-sm pl-7 pr-2 py-2.5 rounded-xl outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all dark:text-gray-300">
                                </div>
                                <span class="text-gray-400 font-medium">-</span>
                                <div class="relative flex-1">
                                    <span class="absolute inset-y-0 left-0 flex items-center pl-3 text-gray-500 font-medium">$</span>
                                    <input type="number" name="max_price" value="{{ request('max_price') }}" placeholder="Max" class="w-full border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-sm pl-7 pr-2 py-2.5 rounded-xl outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all dark:text-gray-300">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="border-t border-gray-100 dark:border-gray-800 pt-5">
                            ...
                            <div class="space-y-2">
                                @for ($r = 4; $r >= 1; $r--)
                                    <label class="flex items-center gap-2 cursor-pointer group">
                                        <input type="radio" name="rating" value="{{ $r }}" @checked(request('rating') == $r) class="h-4 w-4 rounded border-gray-300 dark:border-gray-600 text-primary-600 focus:ring-primary-500">
                                        <div class="flex items-center gap-0.5">
                                            @for ($s = 1; $s <= 5; $s++)
                                                <svg class="w-3.5 h-3.5 {{ $s <= $r ? 'text-amber-400 fill-current' : 'text-gray-200 dark:text-gray-600' }}" viewBox="0 0 20 20" fill="currentColor"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
                                            @endfor
                                        </div>
                        ...
                        <div class="flex gap-2">
                            ...
                            <a href="{{ route('storefront.products.index') }}" class="px-4 py-3 border border-gray-200 dark:border-gray-700 rounded-xl text-sm font-bold text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors" title="Clear all">
            ...
            <div class="flex-1 min-w-0">
                ...
                <div class="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-2xl shadow-sm mb-6 px-5 py-4 flex items-center justify-between">
                    <div class="flex items-center gap-4">
                        <span class="text-lg font-bold text-gray-900 dark:text-white">All Products</span>
                        @if(request('q'))
                            <div class="h-4 w-px bg-gray-300 dark:bg-gray-700"></div>
                            <span class="text-sm text-gray-500 dark:text-gray-400">Results for <span class="font-semibold text-primary-600 dark:text-primary-400">"{{ request('q') }}"</span></span>
                        @endif
                    </div>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                    ...
                    @empty
                        <div class="col-span-full bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-2xl py-20 px-4 text-center shadow-sm">
                            ...
                            <p class="text-sm text-gray-500 dark:text-gray-400 mb-6 max-w-sm mx-auto">We couldn't find anything matching your search criteria. Try modifying your filters or search term.</p>
                            <a href="{{ route('storefront.products.index') }}" class="inline-flex items-center gap-2 text-sm text-primary-600 dark:text-primary-400 font-bold hover:text-primary-700 dark:hover:text-primary-300 hover:underline">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg>
                                Clear all filters
                            </a>
                        </div>
